...
const HoverVideoPlayerThumbnail: React.FC<{ src: string }> = ({ src }) => {
  ...
  const isImageFile = /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(src)

  return (
    <div
      className={cn("absolute inset-0 w-full h-full bg-black/5")}
      style={{
        top: `-${cropTop}%`,
        bottom: `-${cropBottom}%`,
        height: `calc(100% + ${cropTop + cropBottom}%)`,
...
const HoverVideoPlayerProgressBar: React.FC = () => {
  ...
  return (
    <input
      type="range"
      className="mt-2 w-full h-1 bg-white/20 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white"
      value={progress}
      min={0}
      max={100}
      step={0.1}
